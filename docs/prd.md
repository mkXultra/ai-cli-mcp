...
- モデル名から適切なCLI（Claude / Codex / Gemini / OpenCode / Grok）が自動選択される
- Ultra エイリアス（`claude-ultra`, `codex-ultra`, `gemini-ultra`）とユーザー定義エイリアスによるモデル指定をサポート
- `session_id` による前回セッションの継続をサポート（Claude / Codex / Gemini / OpenCode / Grok）
...
- タイムアウト指定が可能（既定: 180秒）。`timeout: 0` / `ai-cli wait --timeout 0` は無期限で全PIDの終了を待つ
- 負数・非数値のタイムアウト指定は拒否する。有限の待機時間を超えても実行中のプロセスは停止しない
...
- `ai-cli alias list` で組み込み・ユーザー定義を統合したエイリアス一覧と設定パスを取得できる。一覧取得では設定ファイルを作成・変更しない
